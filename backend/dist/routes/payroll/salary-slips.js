...
router.get('/calculate/:user_id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const holdRequest = yield prismaClient_1.default.salaryHoldRequest.findFirst({
            where: {
        ...
        if (!currentCTC)
            return res.status(400).json({ error: "No active CTC found for employee" });
...
router.post('/generate', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { user_id, employee_ctc_id, month, year, salary_type, month_working_days, employee_working_days, paid_leaves, unpaid_leaves, total_leaves, paid_holidays, paid_week_offs, extra_days, joining_net_salary, joining_gross_salary, this_month_gross, per_day_salary, per_day_extra, total_earnings, total_deductions, net_salary, salary_mode, status, description, items } = req.body;
        const result = yield prismaClient_1.default.$transaction((tx) => __awaiter(void 0, void 0, void 0, function* () {
            // Check if slip already exists
            const existing = yield tx.salarySlip.findUnique({
            ...
            if (existing) {
                ...
                yield tx.salarySlipItem.deleteMany({ where: { salary_slip_id: existing.id } });
                yield tx.salarySlip.delete({ where: { id: existing.id } });
            }
            const slip = yield tx.salarySlip.create({
                data: {
            ...
            if (items && Array.isArray(items)) {
                yield tx.salarySlipItem.createMany({
                    data: items.map((item) => ({
        ...
        yield (0, activityLogger_1.logActivity)(null, 'GENERATED', 'SALARY_SLIP', `User ID: ${user_id}, ${month}/${year}`);
...
router.get('/list', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const slips = yield prismaClient_1.default.salarySlip.findMany({
            where,
            include: {
                user: {
                    select: {
...
router.post('/bulk-generate', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const log = yield prismaClient_1.default.salaryGenerationLog.create({
            data: {
        ...
        for (const userId of user_ids) {
            try {
                const hold = yield prismaClient_1.default.salaryHoldRequest.findFirst({
                    where: {
                ...
                if (!employee || !employee.employeeCTCs[0])
                    continue;
        ...
        yield prismaClient_1.default.salaryGenerationLog.update({
            where: { id: log.id },
            data: {
                status: 'Completed',
...
router.patch('/publish', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { ids } = req.body; // Array of IDs
        if (!ids || !Array.isArray(ids)) {
            return res.status(400).json({ error: "Invalid IDs provided" });
        }
        yield prismaClient_1.default.salarySlip.updateMany({
            where: { id: { in: ids } },
            data: { status: 'Published' }
        });
        yield (0, activityLogger_1.logActivity)(null, 'PUBLISHED', 'SALARY_SLIP', `Count: ${ids.length}`);
...
router.post('/bulk-delete', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const slips = yield prismaClient_1.default.salarySlip.findMany({ where: { id: { in: ids } } });
        ...
        yield prismaClient_1.default.$transaction([
            prismaClient_1.default.salarySlipItem.deleteMany({ where: { salary_slip_id: { in: ids } } }),
            prismaClient_1.default.salarySlip.deleteMany({ where: { id: { in: ids } } })
        ]);
        yield (0, activityLogger_1.logActivity)(null, 'BULK_DELETED', 'SALARY_SLIP', `Count: ${ids.length}`);
...
router.patch('/:id/share', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const { is_shared } = req.body;
        const slip = yield prismaClient_1.default.salarySlip.update({
            where: { id: parseInt(id) },
            data: { is_shared }
        });
        yield (0, activityLogger_1.logActivity)(null, 'SHARED', 'SALARY_SLIP', `ID: ${id}, Shared: ${is_shared}`);
        res.json(slip);
    }
    catch (error) {
...
router.delete('/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const slip = yield prismaClient_1.default.salarySlip.findUnique({ where: { id: parseInt(id) } });
        if (!slip)
            return res.status(404).json({ error: "Salary slip not found" });
        if (slip.status === 'Published') {
            return res.status(403).json({ error: "Cannot delete a published salary slip" });
        }
        yield prismaClient_1.default.salarySlip.delete({ where: { id: parseInt(id) } });
        yield (0, activityLogger_1.logActivity)(null, 'DELETED', 'SALARY_SLIP', `ID: ${id}`);
...
router.patch('/bulk-share', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        yield (0, activityLogger_1.logActivity)(null, 'BULK_SHARE_UPDATED', 'SALARY_SLIP', `Count: ${ids.length}, Status: ${is_shared}`);
