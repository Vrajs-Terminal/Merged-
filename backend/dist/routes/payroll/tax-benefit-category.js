...
router.get('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const processed = categories.map(cat => (Object.assign(Object.assign({}, cat), { usage_count: Math.floor(Math.random() * 50) + 10 // Placeholder for UX mockup
         })));
        res.json(processed);
    }
    catch (error) {
...
router.post('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const existing = yield prismaClient_1.default.taxBenefitCategory.findUnique({
            where: { section_code }
        });
        if (existing) {
            return res.status(400).json({ error: "A category with this Section Code already exists" });
        }
        const category = yield prismaClient_1.default.taxBenefitCategory.create({
            data: {
                category_name,
                section_code,
...
router.put('/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (section_code) {
            const existing = yield prismaClient_1.default.taxBenefitCategory.findUnique({ where: { section_code } });
            if (existing && existing.id !== id) {
                return res.status(400).json({ error: "Another category is using this Section Code" });
            }
        }
        const updated = yield prismaClient_1.default.taxBenefitCategory.update({
            where: { id },
            data: {
                category_name,
                section_code,
...
router.delete('/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const subCatCount = yield prismaClient_1.default.taxBenefitSubCategory.count({ where: { category_id: id } });
        if (subCatCount > 0) {
        ...
        yield prismaClient_1.default.taxBenefitCategory.delete({ where: { id } });
