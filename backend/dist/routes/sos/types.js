...
router.get('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const types = yield prismaClient_1.default.sosType.findMany({
            include: {
...
router.post('/seed', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (const typeName of selectedTypes) {
            const existing = yield prismaClient_1.default.sosType.findUnique({ where: { name: typeName } });
            if (!existing) {
                const created = yield prismaClient_1.default.sosType.create({
                    data: {
                        name: typeName,
                        isPredefined: true,
                        status: 'Active',
...
router.post('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const existing = yield prismaClient_1.default.sosType.findUnique({
            where: { name: name.trim() }
        });
        if (existing)
            return res.status(409).json({ error: 'An SOS type with this name already exists' });
        const sosType = yield prismaClient_1.default.sosType.create({
            data: {
                name: name.trim(),
...
router.put('/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!(name === null || name === void 0 ? void 0 : name.trim()))
            return res.status(400).json({ error: 'SOS name is required' });
        const existing = yield prismaClient_1.default.sosType.findFirst({
            where: { name: name.trim(), id: { not: id } }
        });
        if (existing)
            return res.status(409).json({ error: 'Another SOS type with this name already exists' });
        const updated = yield prismaClient_1.default.sosType.update({
            where: { id },
            data: {
                name: name.trim(),
                imageUrl: imageUrl || null,
...
router.patch('/:id/toggle', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const id = Number(req.params.id);
        const type = yield prismaClient_1.default.sosType.findUnique({ where: { id } });
        if (!type)
...
router.delete('/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const id = Number(req.params.id);
        const alertCount = yield prismaClient_1.default.sosAlert.count({ where: { sosTypeId: id } });
        if (alertCount > 0) {
            return res.status(400).json({ error: `Cannot delete: ${alertCount} alert(s) are linked to this SOS type.` });
        }
        yield prismaClient_1.default.sosType.delete({ where: { id } });
