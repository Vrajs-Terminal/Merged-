...
router.get('/stats', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const [totalEmployees, totalBranches, totalDepartments, totalZones, presentToday, lateToday, absentToday, recentActivities, recentEmployees, departmentCounts] = yield Promise.all([
        ...
        const [onLeaveToday, halfDayToday, missingPunch, pendingExpenses, shiftChanges, bankChanges, deviceChanges, wfhRequests, pastAttendance, faceChanges, profileChanges, overtimeRequests, shortLeaves, taxDocs] = yield Promise.all([
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            const count = yield prismaClient_1.default.user.count({
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const [present, late] = yield Promise.all([
        ...
        const expenseRequests = yield prismaClient_1.default.expenseRequest.findMany({
