...
export default function ProductCategory() {
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true);
      const response = await productCategoryAPI.getAll(1, 1000);
      const { rows } = extractApiList<ProductCategory>(response.data);
      setCategories(rows);
    } catch (error: any) {
      setMsg({ type: "error", text: error.response?.data?.message || "Failed to load product categories." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await productCategoryAPI.update(editingId, formData);
  ...
  const handleToggleStatus = async (id: number) => {
    try {
      await productCategoryAPI.toggleStatus(id);
      setMsg({ type: "success", text: "Category status updated successfully." });
      await fetchCategories();
    } catch (error: any) {
      setMsg({ type: "error", text: error.response?.data?.message || "Failed to update category status." });
    }
  };
    ...
    <div className="lm-container lm-fade opw-page">
      <div className="lm-card opw-hero">
        <div className="opw-hero-copy">
          <span className="opw-eyebrow"><Tags size={14} /> Catalog structure</span>
          <h2 className="lm-page-title"><Tags size={22} /> Product Categories</h2>
          <p className="lm-page-subtitle">
            Organize the catalog with cleaner category management, clearer status handling, and a more professional maintenance flow.
          </p>
          <div className="opw-hero-pills">
            <span className="opw-hero-pill">Live search</span>
            <span className="opw-hero-pill">Inline status control</span>
            <span className="opw-hero-pill">Focused CRUD workflow</span>
          </div>
        </div>
          ...
          <div className="opw-panel-head">
            <div className="opw-panel-title">
              ...
              <div>
                ...
                <p>Use a clear category label and optional description so catalog mapping stays consistent.</p>
              </div>
            </div>
          </div>
        ...
        <div className="opw-panel-head">
          <div className="opw-panel-title">
            ...
            <div>
              ...
              <p>Maintain product-category structure with clearer visibility over descriptions and lifecycle status.</p>
            </div>
          </div>
          <span className={`opw-panel-badge ${activeCount > 0 ? "is-success" : ""}`}>{filteredCategories.length} visible</span>
        </div>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={4}>
                    <div className="opw-empty">
                      ...
                      <p>Pulling the latest catalog structure from the database.</p>
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={4}>
                    <div className="opw-empty">
                      ...
                      <p>Try a broader search or create a new category to start organizing the catalog.</p>
                    </div>
                  </td>
                </tr>
                ...
                <tr key={category.id}>
                  ...
                  <td>
                    <span className={`opw-status-badge ${getStatusTone(category.status)}`}>
                  ...
                  <td>
                    <div className="opw-row-actions">
                      ...
                      <button type="button" className="opw-row-btn is-muted" onClick={() => void handleToggleStatus(category.id)}>
