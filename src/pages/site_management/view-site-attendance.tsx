...
import { Eye, Search, X, Loader2, Calendar, Download, ChevronLeft, ChevronRight, AlertTriangle } from "lucide-react";
...
interface AttRecord {
  ...
  working_hours: number; late_minutes: number; overtime_hours: number;
...
export default function ViewSiteAttendance() {
  ...
  }, [startDate, endDate, siteId, employeeId]);

  const handleExport = () => {
    const headers = ["Sr", "Employee", "Site", "Date", "In Time", "Out Time", "Hours", "Status", "Late(min)", "Remark"];
    const rows = filtered.map((r, i) => [
      ...
      r.working_hours, r.status, r.late_minutes, r.remark || ""
    ]);
    const csv = [headers, ...rows].map(r => r.join(",")).join("\n");
    const a = document.createElement("a");
    a.href = `data:text/csv;charset=utf-8,${encodeURIComponent(csv)}`;
    a.download = `site-attendance-${startDate}-${endDate}.csv`;
    a.click();
  };
    ...
    <div className="sm-page">
      <div className="sm-header">
        <div className="sm-header__left">
          <div className="sm-header__icon"><Eye size={22} /></div>
          <div>
            <h1 className="sm-header__title">View Site Attendance</h1>
            <p className="sm-header__sub">Historical read-only attendance log for all sites</p>
          </div>
        </div>
      ...
      {/* Filter Panel */}
      <div style={{
        background: "var(--sm-surface)", border: "1px solid var(--sm-border)", borderRadius: "var(--sm-radius)",
        padding: "18px 20px", marginBottom: 18, display: "flex", gap: 14, flexWrap: "wrap", alignItems: "flex-end"
      }}>
        ...
        <button className="sm-btn sm-btn--primary" onClick={fetchData} disabled={loading}>
          {loading ? <Loader2 className="sm-spin" size={16} /> : <Search size={16} />} Get Data
        </button>
      </div>

      <div className="sm-controls">
        <div style={{ fontSize: 13, color: "var(--sm-text-muted)" }}>{records.length} records found</div>
      ...
      <div className="sm-table-wrap">
        {loading ? (
          <div className="sm-loading"><Loader2 className="sm-spin" size={28} /><span>Fetching data…</span></div>
        ) : (
          <table className="sm-table">
            <thead>
              <tr>
                <th>#</th><th>Employee</th><th>Site</th><th>Date</th>
                <th>In Time</th><th>Out Time</th><th>Hours</th>
                <th>Status</th><th>Late</th><th>Remark</th>
              </tr>
            </thead>
            <tbody>
              {paged.length === 0 ? (
                <tr><td colSpan={10} className="sm-empty">
                  {records.length === 0 ? "Click 'Get Data' to fetch attendance records" : "No matching records"}
                </td></tr>
              ) : paged.map((rec, i) => (
                <tr key={rec.id}>
                  ...
                  <td>
                    <span style={{
                      ...
                      background: `${STATUS_COLOR[rec.status] || "#6366f1"}20`,
                      color: STATUS_COLOR[rec.status] || "#6366f1"
                    }}>{rec.status}</span>
                  </td>
                  <td style={{ color: rec.late_minutes > 0 ? "#f59e0b" : "var(--sm-text-muted)" }}>
