...
router.get('/', async (req, res) => {
    try {
        ...
        if (employee_id)
            where.user_id = Number(employee_id);
        // Branch and department filtering requires joining the user or site relation
        if (branch_id)
            where.site = { branch_id: Number(branch_id) };
        if (department_id)
            where.site = { ...where.site, department_id: Number(department_id) };
        const employees = await prismaClient_1.default.siteEmployee.findMany({
            where,
            include: {
...
router.post('/', async (req, res) => {
    try {
        ...
        for (const user_id of user_ids) {
            ...
            const existing = await prismaClient_1.default.siteEmployee.findUnique({
            ...
            const emp = await prismaClient_1.default.siteEmployee.create({
                data: {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const updated = await prismaClient_1.default.siteEmployee.update({
            where: { id: Number(id) },
            data: {
                role,
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await prismaClient_1.default.siteEmployee.delete({
            where: { id: Number(id) }
        });
...
//# sourceMappingURL=site-employees.js.map