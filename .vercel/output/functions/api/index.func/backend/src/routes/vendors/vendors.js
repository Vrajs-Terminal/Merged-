...
router.get('/', async (req, res) => {
    try {
        ...
        const vendors = await prismaClient_1.default.vendor.findMany({
            where,
            include: {
...
router.post('/', async (req, res) => {
    try {
        ...
        const vendor = await prismaClient_1.default.vendor.create({
            data: {
                name: data.name,
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (data.paymentTerms)
            data.paymentTerms = Number(data.paymentTerms);
        const vendor = await prismaClient_1.default.vendor.update({
            where: { id: Number(req.params.id) },
            data: {
                name: data.name,
                categoryId: data.categoryId,
...
router.delete('/:id', async (req, res) => {
    try {
        await prismaClient_1.default.vendor.delete({
            where: { id: Number(req.params.id) }
        });
...
router.post('/bulk', async (req, res) => {
    try {
        ...
        const count = await prismaClient_1.default.vendor.createMany({
            data: vendors.map((v) => ({
                name: v.name,
