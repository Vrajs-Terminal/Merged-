...
router.get('/', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        ...
        const posts = await prismaClient_1.default.timelinePost.findMany({
            where: isAdmin ? {} : {
                status: 'Approved',
                OR: [
                    { audience_type: 'All' },
...
router.post('/', authMiddleware_1.authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const post = await prismaClient_1.default.timelinePost.create({
            data: {
                author_id,
                type: type,
                content: content,
...
router.delete('/:id', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        ...
        await prismaClient_1.default.timelinePost.delete({ where: { id: postId } });
...
router.patch('/:id/status', authMiddleware_1.authenticateToken, async (req, res) => {
    ...
    try {
        const updated = await prismaClient_1.default.timelinePost.update({
