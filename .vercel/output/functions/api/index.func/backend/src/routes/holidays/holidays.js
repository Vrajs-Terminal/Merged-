...
router.get('/', async (req, res) => {
    try {
        ...
        const holidays = await prismaClient_1.default.holiday.findMany({
            where,
            orderBy: { date: 'asc' }
        });
...
router.post('/', async (req, res) => {
    try {
        ...
        const holiday = await prismaClient_1.default.holiday.create({
            data: {
                name,
                date: new Date(date),
                type,
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const updated = await prismaClient_1.default.holiday.update({
            where: { id },
            data: {
                name,
...
router.patch('/:id/toggle', async (req, res) => {
    try {
        const id = Number(req.params.id);
        const holiday = await prismaClient_1.default.holiday.findUnique({ where: { id } });
        if (!holiday)
...
router.delete('/:id', async (req, res) => {
    try {
        const id = Number(req.params.id);
        await prismaClient_1.default.holiday.delete({ where: { id } });
