...
router.get('/:groupId', async (req, res) => {
    try {
        ...
        const members = await prismaClient_1.default.chatGroupMember.findMany({
            where: { groupId },
            include: {
                user: {
                    select: {
...
router.post('/:groupId', async (req, res) => {
    try {
        ...
        const member = await prismaClient_1.default.chatGroupMember.create({
            data: {
                groupId,
        ...
        const fullMember = await prismaClient_1.default.chatGroupMember.findUnique({
            where: { id: member.id },
            include: {
                user: {
...
router.put('/:groupId/:memberId', async (req, res) => {
    try {
        ...
        if (status)
            data.status = status;
        const updated = await prismaClient_1.default.chatGroupMember.update({
            where: { id: memberId },
            data,
            include: {
                user: {
...
router.delete('/:groupId/:memberId', async (req, res) => {
    try {
        const memberId = parseInt(req.params.memberId);
        await prismaClient_1.default.chatGroupMember.delete({
            where: { id: memberId }
        });
