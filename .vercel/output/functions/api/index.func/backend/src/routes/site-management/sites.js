...
router.get('/', async (req, res) => {
    try {
        const sites = await prismaClient_1.default.site.findMany({
            include: {
...
router.post('/', async (req, res) => {
    try {
        ...
        const existing = await prismaClient_1.default.site.findUnique({
            where: {
                name_mobile_no: {
                    name,
                    mobile_no
        ...
        const site = await prismaClient_1.default.site.create({
            data: {
                name,
                contact_name,
                mobile_no,
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const updated = await prismaClient_1.default.site.update({
            where: { id: Number(id) },
            data: {
                name,
                contact_name,
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await prismaClient_1.default.site.delete({
            where: { id: Number(id) }
        });
