        ...
        if (deviceId || macAddress) {
            ...
            if (user.mobileDevice) {
                if (user.mobileDevice.status === 'Bound') {
                    ...
                    // Update last login
                    await prismaClient_1.default.mobileDevice.update({
                ...
                await prismaClient_1.default.mobileDevice.create({
                    data: {
                        userId: user.id,
                        deviceId: deviceId || null,
                        macAddress: macAddress || null,
...
router.post('/setup-admin', async (req, res) => {
    ...
    try {
        ...
        const admin = await prismaClient_1.default.user.create({
            data: {
                name,
                email,
                password_hash,
        ...
        await (0, activityLogger_1.logActivity)(admin.id, 'CREATED', 'USER', admin.name, { role: 'Admin', email });
...
router.post('/add-user', async (req, res) => {
    ...
    try {
        ...
        await (0, activityLogger_1.logActivity)((adminUser === null || adminUser === void 0 ? void 0 : adminUser.id) || null, 'CREATED', 'USER', newUser.name, { role, email });
...
router.delete('/user/:id', async (req, res) => {
    try {
        ...
        const adminUser = req.user;
        await (0, activityLogger_1.logActivity)((adminUser === null || adminUser === void 0 ? void 0 : adminUser.id) || null, 'DELETED', 'USER', `User #${req.params.id}`);
...
router.put('/user/:id/password', async (req, res) => {
    ...
    try {
        ...
        const adminUser = req.user;
        await (0, activityLogger_1.logActivity)((adminUser === null || adminUser === void 0 ? void 0 : adminUser.id) || null, 'UPDATED', 'USER', `User #${req.params.id}`, { action: 'Password Reset' });
...
router.post('/forgot-password', async (req, res) => {
    ...
    try {
        ...
        await (0, activityLogger_1.logActivity)(user.id, 'REQUESTED', 'PASSWORD_RESET', 'User requested password reset OTP');
...
router.post('/reset-password', async (req, res) => {
    ...
    try {
        ...
        await (0, activityLogger_1.logActivity)(user.id, 'UPDATED', 'USER', 'User successfully reset their password via OTP');
...
router.post('/send-login-otp', async (req, res) => {
    ...
    try {
        ...
        await (0, activityLogger_1.logActivity)(user.id, 'REQUESTED', 'LOGIN_OTP', 'User requested login OTP');
...
router.post('/login-with-otp', async (req, res) => {
    ...
    try {
        ...
        // Generate JWT Token
        const token = jsonwebtoken_1.default.sign({ id: user.id, email: user.email, role: user.role }, getJWTSecret(), { expiresIn: '1d' });
        await (0, activityLogger_1.logActivity)(user.id, 'LOGIN', 'USER', 'User logged in via OTP');
        res.json({
            message: 'Login successful',
            token,
            user: {
