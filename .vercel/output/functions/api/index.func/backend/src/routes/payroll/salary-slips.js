...
router.get('/calculate/:user_id', async (req, res) => {
    try {
        ...
        const holdRequest = await prismaClient_1.default.salaryHoldRequest.findFirst({
            where: {
...
router.post('/generate', async (req, res) => {
    try {
        ...
        await (0, activityLogger_1.logActivity)(null, 'GENERATED', 'SALARY_SLIP', `User ID: ${user_id}, ${month}/${year}`);
...
router.post('/bulk-generate', async (req, res) => {
    try {
        ...
        const log = await prismaClient_1.default.salaryGenerationLog.create({
            data: {
        ...
        for (const userId of user_ids) {
            try {
                const hold = await prismaClient_1.default.salaryHoldRequest.findFirst({
                    where: {
        ...
        await prismaClient_1.default.salaryGenerationLog.update({
            where: { id: log.id },
            data: {
                status: 'Completed',
...
router.patch('/publish', async (req, res) => {
    try {
        ...
        await prismaClient_1.default.salarySlip.updateMany({
            where: { id: { in: ids } },
            data: { status: 'Published' }
        });
        await (0, activityLogger_1.logActivity)(null, 'PUBLISHED', 'SALARY_SLIP', `Count: ${ids.length}`);
...
router.post('/bulk-delete', async (req, res) => {
    try {
        ...
        const slips = await prismaClient_1.default.salarySlip.findMany({ where: { id: { in: ids } } });
        ...
        await (0, activityLogger_1.logActivity)(null, 'BULK_DELETED', 'SALARY_SLIP', `Count: ${ids.length}`);
...
router.patch('/:id/share', async (req, res) => {
    try {
        ...
        const slip = await prismaClient_1.default.salarySlip.update({
            where: { id: parseInt(id) },
            data: { is_shared }
        });
        await (0, activityLogger_1.logActivity)(null, 'SHARED', 'SALARY_SLIP', `ID: ${id}, Shared: ${is_shared}`);
        res.json(slip);
    }
    catch (error) {
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const slip = await prismaClient_1.default.salarySlip.findUnique({ where: { id: parseInt(id) } });
        if (!slip)
            return res.status(404).json({ error: "Salary slip not found" });
        if (slip.status === 'Published') {
            return res.status(403).json({ error: "Cannot delete a published salary slip" });
        }
        await prismaClient_1.default.salarySlip.delete({ where: { id: parseInt(id) } });
        await (0, activityLogger_1.logActivity)(null, 'DELETED', 'SALARY_SLIP', `ID: ${id}`);
...
router.patch('/bulk-share', async (req, res) => {
    try {
        ...
        await (0, activityLogger_1.logActivity)(null, 'BULK_SHARE_UPDATED', 'SALARY_SLIP', `Count: ${ids.length}, Status: ${is_shared}`);
...
//# sourceMappingURL=salary-slips.js.map