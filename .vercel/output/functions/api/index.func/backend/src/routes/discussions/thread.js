...
router.get('/:id/comments', async (req, res) => {
    try {
        const { id } = req.params;
        const comments = await prismaClient_1.default.discussionComment.findMany({
...
router.post('/:id/comments', async (req, res) => {
    try {
        ...
        const newComment = await prismaClient_1.default.discussionComment.create({
            data: {
...
router.post('/comments/:id/react', async (req, res) => {
    try {
        ...
        const existing = await prismaClient_1.default.discussionReaction.findFirst({
            where: { comment_id: cId, user_id: uId, reaction }
        });
        if (existing) {
            // Un-react (Delete)
            await prismaClient_1.default.discussionReaction.delete({ where: { id: existing.id } });
            return res.json({ message: 'Reaction removed', active: false });
        }
        else {
            // Add reaction
            await prismaClient_1.default.discussionReaction.create({
