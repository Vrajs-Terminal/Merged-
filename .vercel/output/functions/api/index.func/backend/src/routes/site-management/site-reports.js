...
router.get('/summary', async (req, res) => {
    try {
        ...
        const records = await prismaClient_1.default.siteAttendance.findMany({
            where,
...
router.get('/counts', async (req, res) => {
    try {
        ...
        if (site_id)
            where.siteEmployee = { site_id: Number(site_id) };
        const todayRecords = await prismaClient_1.default.siteAttendance.findMany({
            where
        });
        const counts = {
            present: 0,
...
//# sourceMappingURL=site-reports.js.map