...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const existingBranch = await prismaClient_1.default.branch.findUnique({ where: { code: finalCode } });
        if (existingBranch) {
            return res.status(400).json({ error: 'Branch code already exists' });
        }
        const maxOrder = await prismaClient_1.default.branch.aggregate({
        ...
        const user = req.user;
        await (0, activityLogger_1.logActivity)((user === null || user === void 0 ? void 0 : user.id) || null, 'CREATED', 'BRANCH', branch.name, { code: branch.code, type: branch.type });
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        const branch = await prismaClient_1.default.branch.findUnique({
        ...
        const user = req.user;
        await prismaClient_1.default.branch.delete({ where: { id: parseInt(id) } });
        await (0, activityLogger_1.logActivity)((user === null || user === void 0 ? void 0 : user.id) || null, 'DELETED', 'BRANCH', branch.name);
...
router.post('/bulk', async (req, res) => {
    ...
    try {
        const results = await prismaClient_1.default.$transaction(async (tx) => {
            let count = 0;
        ...
        const user = req.user;
        await (0, activityLogger_1.logActivity)((user === null || user === void 0 ? void 0 : user.id) || null, 'BULK_IMPORT', 'BRANCH', `Imported ${results.count} branches`);
        res.json(results);
    }
    catch (error) {
