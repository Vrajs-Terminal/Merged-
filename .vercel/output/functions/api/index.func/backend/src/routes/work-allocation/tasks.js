...
router.get('/', async (req, res) => {
    try {
        ...
        const tasks = await prismaClient_1.default.workAllocation.findMany({
            where,
            include: {
...
router.post('/', async (req, res) => {
    try {
        ...
        const taskCount = await prismaClient_1.default.workAllocation.count({
        ...
        const access = await prismaClient_1.default.workAllocationAccess.findFirst({
            where: {
        ...
        if (access.max_task_per_day > 0) {
            ...
            const taskAssignedToday = await prismaClient_1.default.workAllocation.count({
                where: {
        ...
        const task = await prismaClient_1.default.workAllocation.create({
            data: {
                task_id,
...
router.patch('/:id/status', async (req, res) => {
    try {
        ...
        const updated = await prismaClient_1.default.workAllocation.update({
            where: { id: Number(id) },
            data: updateData
        });
...
router.patch('/:id/reassign', async (req, res) => {
    try {
        ...
        const task = await prismaClient_1.default.workAllocation.findUnique({
            where: { id: Number(id) }
        });
        if (!task)
