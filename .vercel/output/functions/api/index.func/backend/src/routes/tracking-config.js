...
router.patch('/:userId', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const userId = parseInt(req.params.userId);
        const { enabled, frequency, workingHoursOnly } = req.body;
        const config = await prismaClient_1.default.trackingConfig.upsert({
        ...
        await (0, activityLogger_1.logActivity)(null, 'UPDATED', 'TRACKING_CONFIG', `Updated tracking config for user #${userId}`);
        res.json(config);
    }
    catch (error) {
...
router.post('/bulk', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        ...
        await (0, activityLogger_1.logActivity)(null, 'UPDATED', 'TRACKING_CONFIG', `Bulk ${enabled ? 'enabled' : 'disabled'} ${userIds.length} users`);
...
//# sourceMappingURL=tracking-config.js.map