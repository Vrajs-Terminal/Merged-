...
router.get('/', async (req, res) => {
    try {
        ...
        const records = await prismaClient_1.default.salaryHoldRequest.findMany({
            where,
            include: {
                employee: {
                    select: {
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const record = await prismaClient_1.default.salaryHoldRequest.create({
            data: {
        ...
        await (0, activityLogger_1.logActivity)(((_b = req.user) === null || _b === void 0 ? void 0 : _b.id) || null, 'ADDED', 'SALARY_HOLD_REQUEST', `Employee ID: ${employee_id}, Period: ${month}/${year}`);
...
router.patch('/:id/status', async (req, res) => {
    try {
        ...
        const record = await prismaClient_1.default.salaryHoldRequest.update({
            where: { id: parseInt(id) },
            data: { status }
        });
        await (0, activityLogger_1.logActivity)(null, 'STATUS_UPDATED', 'SALARY_HOLD_REQUEST', `ID: ${id}, Status: ${status}`);
        res.json(record);
    }
    catch (error) {
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const record = await prismaClient_1.default.salaryHoldRequest.findUnique({ where: { id: parseInt(id) } });
        ...
        await prismaClient_1.default.salaryHoldRequest.delete({ where: { id: parseInt(id) } });
        await (0, activityLogger_1.logActivity)(null, 'DELETED', 'SALARY_HOLD_REQUEST', `ID: ${id}`);
...
//# sourceMappingURL=salary-hold.js.map