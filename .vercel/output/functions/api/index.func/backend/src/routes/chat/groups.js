...
router.get('/', async (req, res) => {
    try {
        ...
        const totalItems = await prismaClient_1.default.chatGroup.count({ where });
        let groups = await prismaClient_1.default.chatGroup.findMany({
            where,
            include: {
                _count: {
                    select: { members: true }
        ...
        const groupsWithExtra = await Promise.all(groups.map(async (g) => {
            ...
            if (g.autoBranchId) {
                const b = await prismaClient_1.default.branch.findUnique({ where: { id: g.autoBranchId }, select: { name: true } });
                if (b)
...
router.post('/', async (req, res) => {
    try {
        ...
        const newGroup = await prismaClient_1.default.$transaction(async (tx) => {
            // 1. Create group
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const updated = await prismaClient_1.default.chatGroup.update({
            where: { id },
            data: {
                ...(name && { name }),
                ...(icon !== undefined && { icon }),
...
router.delete('/:id', async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        await prismaClient_1.default.chatGroup.delete({
            where: { id }
        });
