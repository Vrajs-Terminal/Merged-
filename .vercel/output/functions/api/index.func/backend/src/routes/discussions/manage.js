...
router.get('/', async (req, res) => {
    try {
        ...
        const discussions = await prismaClient_1.default.discussion.findMany({
            where,
            include: {
...
router.post('/', async (req, res) => {
    try {
        ...
        const discussion = await prismaClient_1.default.discussion.create({
            data: {
                title,
                description,
                ...
                status: 'Active',
                targetsBranch: {
                    create: (branchIds || []).map((id) => ({ branch_id: Number(id) }))
                },
                targetsDept: {
                    create: (deptIds || []).map((id) => ({ department_id: Number(id) }))
                },
                targetsUser: {
                    create: (userIds || []).map((id) => ({ user_id: Number(id) }))
                },
                attachments: {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const updated = await prismaClient_1.default.discussion.update({
            where: { id: Number(id) },
            data: {
                status,
                title,
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await prismaClient_1.default.discussion.update({
            where: { id: Number(id) },
            data: { status: 'Deleted' }
        });
