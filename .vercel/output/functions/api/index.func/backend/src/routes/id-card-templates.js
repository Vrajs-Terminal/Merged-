...
router.get('/', async (req, res) => {
    try {
        const templates = await prismaClient_1.default.idCardTemplate.findMany({
            include: {
                branch: true,
                department: true,
            },
...
router.post('/', async (req, res) => {
    try {
        ...
        const newTemplate = await prismaClient_1.default.idCardTemplate.create({
            data: {
                template_name,
        ...
        await (0, activityLogger_1.logActivity)(null, 'CREATED', 'ID_CARD_TEMPLATE', template_name);
...
router.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { template_name, template_type, branch_id, department_id, status } = req.body;
        const updatedTemplate = await prismaClient_1.default.idCardTemplate.update({
        ...
        await (0, activityLogger_1.logActivity)(null, 'UPDATED', 'ID_CARD_TEMPLATE', template_name);
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const template = await prismaClient_1.default.idCardTemplate.findUnique({ where: { id: parseInt(id) } });
        await prismaClient_1.default.idCardTemplate.delete({
            where: { id: parseInt(id) }
        });
        if (template) {
            await (0, activityLogger_1.logActivity)(null, 'DELETED', 'ID_CARD_TEMPLATE', template.template_name);
        }
        res.status(204).send();
    }
    catch (error) {
...
//# sourceMappingURL=id-card-templates.js.map