    ...
    try {
        ...
        const result = groups.map((g) => ({
            ...
            earningHeads: g.components
                ...
                .map((c) => c.earningDeductionType.name),
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const group = await prismaClient_1.default.salaryGroup.findUnique({
...
router.post('/', async (req, res) => {
    try {
        ...
        const existing = await prismaClient_1.default.salaryGroup.findUnique({ where: { name } });
        if (existing) {
            return res.status(409).json({ error: `Salary group '${name}' already exists` });
        }
        const group = await prismaClient_1.default.$transaction(async (tx) => {
        ...
        await (0, activityLogger_1.logActivity)(null, 'CREATED', 'SALARY_GROUP', name);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const existing = await prismaClient_1.default.salaryGroup.findFirst({
        ...
        await (0, activityLogger_1.logActivity)(null, 'UPDATED', 'SALARY_GROUP', name);
        res.json(group);
    }
    catch (error) {
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const group = await prismaClient_1.default.salaryGroup.delete({
            where: { id: parseInt(id) }
        });
        await (0, activityLogger_1.logActivity)(null, 'DELETED', 'SALARY_GROUP', group.name);
...
router.post('/:id/copy', async (req, res) => {
    try {
        const { id } = req.params;
        const original = await prismaClient_1.default.salaryGroup.findUnique({
            where: { id: parseInt(id) },
        ...
        let counter = 1;
        while (await prismaClient_1.default.salaryGroup.findUnique({ where: { name: copyName } })) {
            counter++;
            copyName = `${original.name} (Copy ${counter})`;
        }
        const copied = await prismaClient_1.default.$transaction(async (tx) => {
            var _a, _b, _c, _d, _e;
        ...
        await (0, activityLogger_1.logActivity)(null, 'CREATED', 'SALARY_GROUP', `${copyName} (copied from ${original.name})`);
...
router.patch('/:id/toggle', async (req, res) => {
    try {
        ...
        const updated = await prismaClient_1.default.salaryGroup.update({
            where: { id: parseInt(id) },
            data: { status: newStatus }
        });
        await (0, activityLogger_1.logActivity)(null, 'UPDATED', 'SALARY_GROUP', `${group.name} → ${newStatus}`);
        res.json(updated);
    }
    catch (error) {
...
//# sourceMappingURL=salary-groups.js.map