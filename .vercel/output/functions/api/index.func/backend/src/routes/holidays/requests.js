...
router.get('/', async (req, res) => {
    try {
        ...
        if (status)
            where.status = status;
        const requests = await prismaClient_1.default.optionalHolidayRequest.findMany({
            where,
            include: {
...
router.post('/', async (req, res) => {
    try {
        ...
        const holiday = await prismaClient_1.default.holiday.findUnique({ where: { id: holidayId } });
        if (!holiday || holiday.type !== 'Optional') {
            return res.status(400).json({ error: 'This is not an optional holiday' });
        }
        const request = await prismaClient_1.default.optionalHolidayRequest.create({
            data: {
...
router.patch('/:id/status', async (req, res) => {
    try {
        ...
        const updated = await prismaClient_1.default.optionalHolidayRequest.update({
            where: { id },
            data: {
                status,
