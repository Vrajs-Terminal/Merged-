...
// Get all beat plans
const getAllBeatPlans = async (req, res) => {
    try {
...
const getBeatPlan = async (req, res) => {
    try {
        const { id } = req.params;
        const beatPlan = await prismaClient_1.default.beatPlan.findUnique({
...
const createBeatPlan = async (req, res) => {
    try {
        ...
        const beatPlan = await prismaClient_1.default.beatPlan.create({
            data: {
...
const updateBeatPlan = async (req, res) => {
    try {
        ...
        const beatPlan = await prismaClient_1.default.beatPlan.update({
            where: { id: parseInt(id) },
            data: {
                ...(weekDay && { weekDay }),
...
// Delete beat plan
const deleteBeatPlan = async (req, res) => {
    try {
        const { id } = req.params;
        await prismaClient_1.default.beatPlan.delete({
            where: { id: parseInt(id) }
        });
...
// Toggle status
const toggleBeatPlanStatus = async (req, res) => {
    try {
        const { id } = req.params;
        const beatPlan = await prismaClient_1.default.beatPlan.findUnique({
            where: { id: parseInt(id) }
        ...
        const updated = await prismaClient_1.default.beatPlan.update({
            where: { id: parseInt(id) },
...
//# sourceMappingURL=beatPlanController.js.map