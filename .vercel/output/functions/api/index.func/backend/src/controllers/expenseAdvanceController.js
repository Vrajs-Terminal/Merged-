...
const getAdvances = async (req, res) => {
    try {
        const advances = await prismaClient_1.default.expenseAdvance.findMany({
...
const createAdvance = async (req, res) => {
    try {
        ...
        const advance = await prismaClient_1.default.expenseAdvance.create({
            data: {
...
const approveAdvance = async (req, res) => {
    try {
        ...
        const adv = await prismaClient_1.default.expenseAdvance.findUnique({ where: { id: Number(id) } });
        if (!adv)
            return res.status(404).json({ error: 'Not found' });
        const updated = await prismaClient_1.default.expenseAdvance.update({
            where: { id: Number(id) },
            data: {
                status: 'Approved',
...
//# sourceMappingURL=expenseAdvanceController.js.map